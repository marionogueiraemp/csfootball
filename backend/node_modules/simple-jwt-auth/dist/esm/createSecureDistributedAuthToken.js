 ...
 */
export const createSecureDistributedAuthToken = ({ headerClaims, claims, privateKey, }) => {
    ...
    if (!claims.iss)
        throw new SimpleJwtAuthError('token.claims.iss must be defined when creating a secure token');
    if (!claims.aud)
        throw new SimpleJwtAuthError('token.claims.aud must be defined when creating a secure token');
    // check that expiration has been defined
    if (!claims.exp)
        throw new SimpleJwtAuthError('token.claims.exp must be defined when creating a secure token');
    // create a token
    const payload = [
        base64UrlEncode(JSON.stringify(headerClaims)),
