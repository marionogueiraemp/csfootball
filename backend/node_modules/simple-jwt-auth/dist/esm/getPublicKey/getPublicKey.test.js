...
const cachePublicKeyMock = cachePublicKey;
const getPublicKeyFromCacheMock = getPublicKeyFromCache;
jest.mock('./discoverJwksUriFromAuthServerMetadata');
const discoverJwksUriFromAuthServerMetadataMock = discoverJwksUriFromAuthServerMetadata;
discoverJwksUriFromAuthServerMetadataMock.mockReturnValue('__discovered_jwks_uri__');
jest.mock('./extractPublicKeyFromJwksUri');
const extractPublicKeyFromJwksUriMock = extractPublicKeyFromJwksUri;
...
describe('getPublicKey', () => {
    ...
    given('the token is not already cached', () => {
        ...
        when('the jwks uri needs to be discovered', () => {
            it('the jwks uri discovery flow should be invoked', () => __awaiter(void 0, void 0, void 0, function* () {
                yield getPublicKey({ token });
                expect(discoverJwksUriFromAuthServerMetadataMock).toHaveBeenCalledTimes(1);
            }));
            then('the public key should be extracted from the discovered jwks uri', () => __awaiter(void 0, void 0, void 0, function* () {
                yield getPublicKey({ token });
                expect(extractPublicKeyFromJwksUri).toHaveBeenCalledTimes(1);
                expect(extractPublicKeyFromJwksUri).toHaveBeenCalledWith(expect.objectContaining({ jwksUri: '__discovered_jwks_uri__' }));
            }));
        });
        when('the jwks uri is explicitly defined', () => {
            then('the jwks uri discovery flow should be skipped', () => __awaiter(void 0, void 0, void 0, function* () {
                yield getPublicKey({ token, jwksUri: '__explicit_jwks_uri__' });
                expect(discoverJwksUriFromAuthServerMetadataMock).toHaveBeenCalledTimes(0);
            }));
            then('the public key should be extracted from then defined jwks uri', () => __awaiter(void 0, void 0, void 0, function* () {
                yield getPublicKey({ token, jwksUri: '__explicit_jwks_uri__' });
                expect(extractPublicKeyFromJwksUri).toHaveBeenCalledTimes(1);
                expect(extractPublicKeyFromJwksUri).toHaveBeenCalledWith(expect.objectContaining({ jwksUri: '__explicit_jwks_uri__' }));
            }));
        });
        then('the token should be cached for subsequent lookups', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    given('the token is already cached', () => {
        ...
        then('the jwks uri discovery flow should not be invoked', () => __awaiter(void 0, void 0, void 0, function* () {
            yield getPublicKey({ token });
            expect(discoverJwksUriFromAuthServerMetadataMock).toHaveBeenCalledTimes(0);
        }));
        then('the public key extraction flow should not be invoked', () => __awaiter(void 0, void 0, void 0, function* () {
            yield getPublicKey({ token });
            expect(extractPublicKeyFromJwksUri).toHaveBeenCalledTimes(0);
        }));
        then('it should return the cached public key', () => __awaiter(void 0, void 0, void 0, function* () {
            const publicKey = yield getPublicKey({
                token,
            });
