...
import { discoverJwksUriFromAuthServerMetadata, DiscoverJwksUriFromAuthServerMetadataError, } from './discoverJwksUriFromAuthServerMetadata';
jest.mock('axios');
...
describe('discoverJwksUriFromAuthServerMetadata', () => {
    ...
    when(`a token's issuer is not a public host`, () => {
        ...
        then('we should throw an error to avoid executing an `http:get` on random strings', () => __awaiter(void 0, void 0, void 0, function* () {
            try {
                yield discoverJwksUriFromAuthServerMetadata({ claims });
                throw new Error('should not reach here');
            }
            catch (error) {
                expect(error).toBeInstanceOf(DiscoverJwksUriFromAuthServerMetadataError);
    ...
    when('the token.issuer !== auth server metadata.issuer', () => {
        ...
        then('we should throw an error since this could be a security risk', () => __awaiter(void 0, void 0, void 0, function* () {
            const error = yield getError(discoverJwksUriFromAuthServerMetadata({ claims }));
            expect(error).toBeInstanceOf(DiscoverJwksUriFromAuthServerMetadataError);
    ...
    when('it successfully finds the jwks uri', () => {
        ...
        then('it should return the jwks uri it found', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const claims = getUnauthedClaims({ token });
            const jwksUri = yield discoverJwksUriFromAuthServerMetadata({
                claims,
            });
