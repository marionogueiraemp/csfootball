 ...
 */
export const getTokenFromAuthorizationCookieWithCSRFProtection = ({ headers, }) => {
    var _a, _b, _c;
    // attempt to grab the jwt from the auth cookie
    const token = getTokenFromAuthorizationCookie({ headers });
    if (!token)
        return null; // if no token in cookie, do nothing
    // check for CSRF, checking that the "source origin" is samesite as "target origin"
    const sourceOrigin = (_c = (_b = (_a = headers.origin) !== null && _a !== void 0 ? _a : headers.Origin) !== null && _b !== void 0 ? _b : headers.referrer) !== null && _c !== void 0 ? _c : headers.Referrer;
    if (!sourceOrigin)
        throw new PotentialCSRFAttackError({
            reason: 'source origin can not be detected from request. no origin or referrer.',
        });
...
//# sourceMappingURL=getTokenFromAuthorizationCookieWithCSRFProtection.js.map