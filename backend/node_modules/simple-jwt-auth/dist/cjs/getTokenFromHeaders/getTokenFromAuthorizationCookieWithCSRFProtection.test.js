...
const PotentialCSRFVulnerabilityError_1 = require("./PotentialCSRFVulnerabilityError");
const PotentialXSSVulnerabilityError_1 = require("./PotentialXSSVulnerabilityError");
...
const exampleAntiCSRFToken = (0, redactSignature_1.redactSignature)({ token: exampleToken });
describe('getTokenFromAuthorizationCookieWithCSRFProtection', () => {
    it('should not get the token from the cookie, if cookie is not set', () => {
        const headers = {};
        const token = (0, getTokenFromAuthorizationCookieWithCSRFProtection_1.getTokenFromAuthorizationCookieWithCSRFProtection)({
            headers,
        });
    ...
    describe('Verifying Origin with Standard Headers', () => {
        it('should throw a CSRFAttemptError if the headers.origin is different from target origin', () => {
            ...
            try {
                (0, getTokenFromAuthorizationCookieWithCSRFProtection_1.getTokenFromAuthorizationCookieWithCSRFProtection)({ headers });
            }
            catch (error) {
                expect(error).toBeInstanceOf(PotentialCSRFAttackError_1.PotentialCSRFAttackError);
                expect(error.message).toEqual(`Potential cross-site-request-forgery attack detected!!! source origin is not same site as target origin! (target='https://api.whodis.io', source='https://hakrsite.com')`);
            }
        });
        ...
        it('should throw a CSRFAttemptError if neither header.origins nor header.referrer is set', () => {
            ...
            try {
                ...
                expect(error).toBeInstanceOf(PotentialCSRFAttackError_1.PotentialCSRFAttackError);
                expect(error.message).toEqual(`Potential cross-site-request-forgery attack detected!!! source origin can not be detected from request. no origin or referrer.`);
            }
        });
    });
    describe('Token Based Mitigation', () => {
        ...
        it('should throw a XSSVulnerabilityError if the anti-csrf-token does not have a redacted signature', () => {
            ...
            try {
                ...
                expect(error).toBeInstanceOf(PotentialXSSVulnerabilityError_1.PotentialXSSVulnerabilityError); // XSS Vulnerability, since user's js should _never_ have the real token. that's the point of putting it into the cookie, to prevent the token from getting stolen with XSS
        ...
        it('should throw a CSRFAttemptError if the header claims of the anti-csrf-token do not match the auth token, since not synchronized', () => {
            ...
            const antiCSRFToken = (0, redactSignature_1.redactSignature)({ token: differentExampleToken });
            const headers = {
                cookie: `authorization=${differentExampleToken}`,
        ...
        it('should throw a CSRFVulnerabilityError if the token does not have a jti', () => {
            ...
            const antiCSRFToken = (0, redactSignature_1.redactSignature)({ token: exampleTokenWithoutJTI });
            const headers = {
            ...
            try {
                ...
                expect(error).toBeInstanceOf(PotentialCSRFVulnerabilityError_1.PotentialCSRFVulnerabilityError);
...
//# sourceMappingURL=getTokenFromAuthorizationCookieWithCSRFProtection.test.js.map