"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.exampleEccKeyPair = exports.exampleRsaKeyPair = void 0;
const test_fns_1 = require("test-fns");
const createVerifiableSignature_1 = require("./createVerifiableSignature");
const isSignatureVerified_1 = require("./isSignatureVerified");
exports.exampleRsaKeyPair = {
    publicKey: `
-----BEGIN PUBLIC KEY-----

-----END PUBLIC KEY-----
  `.trim(),
    privateKey: `
-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
  `.trim(),
};
exports.exampleEccKeyPair = {
    publicKey: `
-----BEGIN PUBLIC KEY-----
MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEaZNCzOoTullC2d4ZCJ0fdJZBefck
ZzQca3OHKK9a2+W6F+E5A81GfL2Z80S3Pr/AOrgTb7K+xCcWmwrmhzsl/A==
-----END PUBLIC KEY-----
  `.trim(),
    privateKey: `
-----BEGIN PRIVATE KEY-----
MIGHAgEAMBMGByqGSM49AgEGCCqGSM49AwEHBG0wawIBAQQgIRi530vfFLrb3JN9
DhgLACRvDhWnnlCfacwzXlgBDr2hRANCAARpk0LM6hO6WULZ3hkInR90lkF59yRn
NBxrc4cor1rb5boX4TkDzUZ8vZnzRLc+v8A6uBNvsr7EJxabCuaHOyX8
-----END PRIVATE KEY-----
  `.trim(),
};
describe('createVerifiableSignature', () => {
    const payload = 'hello';
    (0, test_fns_1.given)('rsa algorithm', () => {
        const alg = 'RS256';
        (0, test_fns_1.when)('provided an rsa key', () => {
            (0, test_fns_1.then)('create verifiable signature', () => {
                const signature = (0, createVerifiableSignature_1.createVerifiableSignature)({
                    alg,
                    payload,
                    privateKey: exports.exampleRsaKeyPair.privateKey,
                });
                // check that its deterministic
                expect(signature).toMatchSnapshot(); // should be deterministic
                // check that it is verifiable
                const verified = (0, isSignatureVerified_1.isSignatureVerified)({
                    alg,
                    payload,
                    signature,
                    publicKey: exports.exampleRsaKeyPair.publicKey,
                });
                expect(verified).toEqual(true);
            });
        });
        (0, test_fns_1.when)('provided an ecc key', () => {
            // TODO: add additional safety by throwing an error if wrong key type is given
            test_fns_1.then.skip('throw an error', () => __awaiter(void 0, void 0, void 0, function* () {
                const error = (0, test_fns_1.getError)(() => (0, createVerifiableSignature_1.createVerifiableSignature)({
                    alg,
                    payload,
                    privateKey: exports.exampleEccKeyPair.privateKey,
                }));
                expect(error.message).toContain('todo');
            }));
        });
    });
    (0, test_fns_1.given)('ecc algorithm', () => {
        const alg = 'ES256';
        (0, test_fns_1.when)('provided an ecc key', () => {
            (0, test_fns_1.then)('create verifiable signature', () => __awaiter(void 0, void 0, void 0, function* () {
                const signature = (0, createVerifiableSignature_1.createVerifiableSignature)({
                    alg,
                    payload,
                    privateKey: exports.exampleEccKeyPair.privateKey,
                });
                // note that ecc signatures are _not_ deterministic, so just sanity check the length looks right
                expect(signature.length).toEqual(86);
                // check that it is verifiable
                const verified = (0, isSignatureVerified_1.isSignatureVerified)({
                    alg,
                    payload,
                    signature,
                    publicKey: exports.exampleEccKeyPair.publicKey,
                });
                expect(verified).toEqual(true);
            }));
        });
        (0, test_fns_1.when)('provided an rsa key', () => {
            // TODO: add additional safety by throwing an error if wrong key type is given
            test_fns_1.then.skip('throw an error', () => __awaiter(void 0, void 0, void 0, function* () {
                const error = (0, test_fns_1.getError)(() => (0, createVerifiableSignature_1.createVerifiableSignature)({
                    alg,
                    payload,
                    privateKey: exports.exampleRsaKeyPair.privateKey,
                }));
                expect(error.message).toContain('todo');
            }));
        });
    });
});
//# sourceMappingURL=createVerifiableSignature.test.js.map